...
class Signal(object):
    ...
    def play(self):
        ymax = np.max(np.abs(self.ys))
        print(f'Max: {ymax} Min: {self.ys.min()}')
        y = (0.5 * self.ys / ymax * 32768).astype(np.int16)  # scale to int16 for sound card
        pygame.mixer.pre_init(self.rate, size=-16, channels=1)
        pygame.mixer.init()
